const request = require("supertest"), app = require("../app"), fs = require("fs"), path = require("path");
...
beforeAll(async () => {
    await app.init();
    agent = request.agent(app).set("api-key", process.env.ADMIN_KEY);
}, 30000);
...
afterAll(async () => {
    const failed = results.length === 0 || results.some(r => !r.passed);
    const logLine = (failed ? "Tests Failed" : "Tests Passed") + " | Details: " + results.map(r => r.passed ? `${r.name} - Passed` : `${r.name} - Failed: ${r.error}`).join(", ");
    log(logLine);

    try { await disconnectDb(); }
    catch (err) { console.error("Failed to disconnect from DB:", err.message); }

    if (agent && process.env.NODE_ENV !== "development" && process.env.NODE_ENV !== "test") {
        try {
            await agent.post("/api/admin/notify").send({
                title: process.env.NODE_ENV.toUpperCase() + (failed ? " : Tests Failed" : " : Tests Passed"),
                message: logLine,
                priority: failed ? "5" : "3",
                actions: [{ action: "view", label: "view logs", url: "https://storcale-api.omegadev.xyz/admin-ui/dashboard", clear: false }],
                click: "",
                email: ""
            }).expect(200);
        } catch (err) {
...
describe("General", () => {
    ...
    test("Health: API up", async () => {
        await runTest("Health: API up", async () => {
            await request(app).get("/api/health").expect(200);
        });
    });

    test("Api key required", async () => {
        await runTest("Api key required", async () => {
            await request(app).get("/api/admin/logs").expect(401);
        });
    });

    test("Public route open", async () => {
        await runTest("Public route open", async () => {
            await request(app).get("/admin-ui/dashboard").expect(200);
        });
    });
...
describe("TNIV/DB", () => {
    ...
    describe("match", () => {
        ...
        test("Get matches", async () => {
            await runTest("Get matches", async () => {
                await agent.get("/api/tniv/db/match").expect(200).expect(res => {
                    if (!Array.isArray(res.body.body)) throw new Error("Response does not contain matches");
                });
            });
        }, 30000);

        test("Add match", async () => {
            await runTest("Add match", async () => {
                const json = JSON.parse(fs.readFileSync(path.join(__dirname, "exampleMatch.json"), "utf8"));
                await agent.post("/api/tniv/db/match").send(json).expect(200).expect(res => {
                    if (!res.body || !res.body.code) throw new Error("Missing sessionId");
        ...
        test("Delete match", async () => {
            await runTest("Delete match", async () => {
                await agent.delete("/api/tniv/db/match").send({ sessionId }).expect(200);
            });
        }, 30000);
    ...
    describe("events", () => {
        ...
        test("get Events", async () => {
            await runTest("get Events", async () => {
                await agent.get("/api/tniv/sheets/events").expect(200).expect(res => {
                    if (!Array.isArray(res.body.main) || res.body.main.length === 0) throw new Error("Invalid events response");
                });
            });
        }, 30000);

        test("get Event Stats", async () => {
            await runTest("get Event Stats", async () => {
                await agent.get("/api/tniv/sheets/events/stats").expect(200).expect(res => {
                    if (!res.body || !res.body.stats || res.body.stats.EventCount < 1) throw new Error("Invalid event stats");
                });
            });
        }, 30000);
...
describe("EIC/Case", () => {
    test("Add a Case", async () => {
        await runTest("Add a Case", async () => {
            const jason = JSON.parse('{"caseId": 0,"robloxUsername": "bacon", "robloxId": 1,"weaponMechanic": "lightsabers"}')
            await agent.post("/api/eic/case").send(jason).expect(200)
        })
    })
