...
notify_failure() {
    ...
    local ntfy_message
    ntfy_message="Storcale API deployment failed on $(hostname) (deploy.sh).
Failed command: ${failed_cmd}
Exit code: ${exit_code}"

    if [ -n "$NOTIFY_URL" ]; then
        curl -s \
            -H "Title: TNIV API | Deployment failed" \
            -H "Priority: urgent" \
            -d "$ntfy_message" \
